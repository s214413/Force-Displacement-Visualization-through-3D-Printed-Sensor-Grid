...
% Set up number of modules, grid size and space between readings
ns = mx * my;
gz = specs.grid_size;
sp = floor(gz/7) - 1;
...
if my > 1 || mx > 1
    if my > mx
        ymin = 0; ymax = gz * mx + sp * (mx - 1);
        xmin = 0; xmax = gz * my + sp * (my - 1);
    else
        ymin = 0; ymax = gz * my + sp * (my - 1);
        xmin = 0; xmax = gz * mx + sp * (mx - 1);
    end
else
    ymin = 0; ymax = gz * mx;
    xmin = 0; xmax = gz * my;
...
while ishandle(plotGraph)
    ...
    if ns >= 2
        ...
        if ns == 2
            ...
            Z((x2 + sp*x2 - sp) + gz + sp, y2 + sp*y2 - sp) = F2(x2, y2);
        else
            F2(x2, y2) = str2double(str(F2_index+3:F3_index-1));
            Z((x2 + sp*x2 - sp) + gz + sp, y2 + sp*y2 - sp) = F2(y2, 9 - x2);
        end
    end
    ...
    if ns >= 3
        ...
        if ns == 3
            ...
            Z((x3 + sp*x3 - sp) + 2 * gz + 2 * sp, (y3 + sp*y3 - sp)) = F3(x3, y3);
        elseif ns == 4 && my ~= mx
            ...
            F3(x3, y3) = str2double(str(F3_index+3:F4_index-1));
            Z(x3 + sp*x3 - sp, (y3 + sp*y3 - sp) + gz + sp) = F3(9 - y3, x3);
        end
    end
    ...
    if ns == 4
        ...
        if my ~= mx
            Z((x4 + sp*x4 - sp) + 3 * gz + 3 * sp, (y4 + sp*y4 - sp)) = F4(x4, y4);
        else
            Z((x4 + sp*x4 - sp) + gz + sp, (y4 + sp*y4 - sp) + gz + sp) = F4(9 - x4,9 - y4);
        end
    end
